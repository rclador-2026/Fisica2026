...
# URL de tu Google Apps Script (asegúrate de que termine en /exec)
...
def gemini_generate(prompt):
    try:
        # Configurado específicamente para la versión 2.5 Flash
        ...
        return "El profesor está en el laboratorio. Intenta en un minuto."

def guardar_en_sheets(alumno, tema, tipo, consulta):
    payload = {"alumno": str(alumno), "tema": tema, "tipo": tipo, "consulta": consulta}
    try:
        # Timeout corto para no bloquear el bot si Sheets demora
...
def webhook():
    ...
    # 3. SELECCIÓN DE TEMA -> MOSTRAR ACCIONES (Técnica Anti-Amnesia)
